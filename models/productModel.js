...
productSchema.pre(/^findOneAnd/, async function(next) {
  this.productToDelete = await this.findOne(); // Assining the deleted product to a variable , so that we can access later
  next();
});

productSchema.post(/^findOneAnd/, async function(doc, next) {
  if (this.productToDelete) {
    this.productToDelete.constructor.calcTotalProducts(
      this.productToDelete.productCategory,
      this.productToDelete.productSubCategory
    );
  }
  next();
});
